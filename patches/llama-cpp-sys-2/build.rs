...
fn main() {
    ...
    // Filter out CMAKE_HIP_COMPILER when hip feature is enabled since we configure it ourselves
    for (key, value) in env::vars() {
        if key.starts_with("CMAKE_") {
            // Skip HIP compiler from env if we're going to configure it
            if cfg!(feature = "hip") && key == "CMAKE_HIP_COMPILER" {
                continue;
            }
    ...
    // HIP/ROCm support for AMD GPUs
    if cfg!(feature = "hip") {
        config.define("GGML_HIP", "ON");

        // Force Position Independent Code for all compiled objects
        // Required for linking HIP objects into Rust binaries/shared libraries
        config.define("CMAKE_POSITION_INDEPENDENT_CODE", "ON");

        // Enable Flash Attention and compile ALL template instantiations
        // Required to avoid undefined symbol errors for tile cases like <64, 64>
        config.define("GGML_CUDA_FA", "ON");
        ...
        config.define("CMAKE_PREFIX_PATH", &rocm_path);

        // CMake's native HIP support requires ROCm's clang directly, not hipcc wrapper
        let rocm_clang = format!("{}/llvm/bin/clang++", rocm_path);
        if Path::new(&rocm_clang).exists() {
            config.define("CMAKE_HIP_COMPILER", &rocm_clang);
        }

        // Set GPU architecture - default to gfx1100 (RDNA 3) if not specified
        let gpu_targets = env::var("AMDGPU_TARGETS").unwrap_or_else(|_| "gfx1100".to_string());
        config.define("AMDGPU_TARGETS", &gpu_targets);
        config.define("CMAKE_HIP_ARCHITECTURES", &gpu_targets);

        debug_log!(
            "HIP/ROCm support enabled with ROCM_PATH={}, AMDGPU_TARGETS={}",
            rocm_path,
            gpu_targets
        );
    }
    ...
    // HIP/ROCm library linking for AMD GPUs
    if cfg!(feature = "hip") && !build_shared_libs {
        ...
        // Link ROCm libraries dynamically (static linking not well supported)
        println!("cargo:rustc-link-lib=amdhip64");
        println!("cargo:rustc-link-lib=hipblas");
        println!("cargo:rustc-link-lib=rocblas");

        debug_log!("HIP linking configured with lib path: {}", rocm_lib);
    }
