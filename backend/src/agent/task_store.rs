//! Autonomous Task Store - SQLite persistence for task queue
//!
//! Survives restarts so Trinity can resume work.
...
use crate::agent::autonomous::{AutonomousTask, TaskPriority, TaskStatus, TaskType};
...
impl TaskStore {
    ...
    pub fn save_task(&self, task: &AutonomousTask) -> Result<()> {
        ...
        conn.execute(
            ...
            (id, name, description, priority, status, task_type_json, created_at, started_at, completed_at)
    ...
    pub fn load_pending_tasks(&self) -> Result<Vec<AutonomousTask>> {
        ...
        let mut stmt = conn.prepare(
            r#"
            SELECT id, name, description, priority, task_type_json, created_at
        ...
        let tasks = stmt
            ...
            .filter_map(|r| r.ok())
            .filter_map(
                |(id_str, name, desc, priority_int, type_json, created_str)| {
