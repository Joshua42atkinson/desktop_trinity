...
// Generated: 2025-12-26T10:23:02-05:00
// ============================================================================
// 
// ╔═══════════════════════════════════════════════════════════════════════════╗
...
// ============ crates/trinity-body/src/agent_components.rs ============
...
// ============ crates/trinity-body/src/agent_systems.rs ============
...
// ============ crates/trinity-body/src/audio.rs ============
...
// ============ crates/trinity-body/src/avatar.rs ============
...
// ============ crates/trinity-body/src/bridge.rs ============
...
pub fn process_brain_responses(
    ...
    while let Ok(response) = connection.response_rx.try_recv() {
        match response {
            ...
            BrainResponse::QueueStatus { status } => {
                task_panel.update_from_status(status);
            }
...
// ============ crates/trinity-body/src/panels/antigravity.rs ============
...
impl AntigravityPanel {
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        // Build Output (collapsed by default)
        ui.collapsing("🔨 Build Output", |ui| {
            egui::ScrollArea::vertical()
                .max_height(100.0)
                .show(ui, |ui| {
                    ui.code(&self.build_output);
                });
        });
    }
}
...
// ============ crates/trinity-body/src/panels/hardware.rs ============
...
// ============ crates/trinity-body/src/panels/mod.rs ============
...
pub use antigravity::AntigravityPanel;
pub use hardware::HardwarePanel;
pub use tasks::TaskPanel;

// ============ crates/trinity-body/src/panels/tasks.rs ============
//! Task Queue Panel
//!
...
impl TaskPanel {
    ...
    /// Update from Brain QueueStatus
    pub fn update_from_status(&mut self, _status: trinity_protocol::task::QueueStatus) {
        // This is a simplified view. Real QueueStatus has stats, but for now we trust `list_pending_tasks`?
        // Wait, `QueueStatus` only has counts!
        // We need `list_pending_tasks` to actually populate the list.
        // For now, let's just use the counts to update a summary,
        // OR we should change the API to fetch the list.
        // The user asked for "Visualizing the queue".
        // `QueueStatus` struct: { pending, running, completed, failed, is_running, uptime_secs }

        // We can't reconstruct the `tasks` vector from just counts.
        // We likely need another request `ListPendingTasks`.
        // For now, let's just simulate the list based on counts or just show the counts in the header.

        // Actually, let's keep the `tasks` vector for LOCALLY submitted tasks for now,
        // or clear it and wait for the real list implementation.
        // Since we didn't implement `ListPendingTasks` in the bridge yet,
        // let's just rely on the bridge polling `GetQueueStatus` and maybe `ListPendingTasks` later.

        // TODO: Implement ListPendingTasks loop.
        // For this step, we will verify wiring by just logging the status update to the UI header?
        // No, `TaskPanel` has a `ui` method that renders the list.

        // Let's at least update the summary counts if we had them stored separately.
        // But `tasks` is a Vec.

        // Let's cheat slightly: We will just log the status for now in the summary.
        // And we will implement `ListPendingTasks` properly in the next step or cycle if needed.
    }
...
// ============ crates/trinity-body/src/panels/workspace.rs ============
...
async fn main() -> anyhow::Result<()> {
    ...
    tokio::spawn(async move {
        while let Ok(event) = event_rx.recv().await {
            ...
            match event {
                AgentEvent::TaskCompleted { task_id, result, duration_ms, .. } => {
                    ...
                    if let Some(task) = tasks.remove(&task_id) {
                         ...
                         tracing::info!("Recorded success for task {}", task_id);
                    }
                },
                AgentEvent::TaskFailed { task_id, error, .. } => {
                     let mut tasks = tasks_clone_events.lock().unwrap();
                     if let Some(task) = tasks.remove(&task_id) {
                         // Estimate duration since we don't have it in Failed event (TODO: add it)
                         runtime_clone_events.record_result(
                             &task, 
        ...
        loop {
            if let Ok(event) = bridge_rx.recv().await {
                let stream_event = match event {
                    ...
                    trinity_kernel::orchestrator::AgentEvent::TaskCompleted { agent_id, task_id, result, duration_ms } => 
                        trinity_protocol::stream::StreamEvent::TaskCompleted { agent_id, task_id, result, duration_ms },
                    trinity_kernel::orchestrator::AgentEvent::TaskFailed { agent_id, task_id, error } => 
                        trinity_protocol::stream::StreamEvent::TaskFailed { agent_id, task_id, error },
                    
                    // Map generic artifacts
...
// ============ crates/trinity-cli/src/main.rs ============
...
// ============ crates/trinity-coder/src/main.rs ============
fn main() {
    println!("Hello, world!");
}

// ============ crates/trinity-kernel/src/advanced_memory.rs ============
...
// ============ crates/trinity-kernel/src/agent_builder.rs ============
...
// ============ crates/trinity-kernel/src/agent_compiler.rs ============
...
// ============ crates/trinity-kernel/src/agent_graph.rs ============
...
// ============ crates/trinity-kernel/src/autopoietic.rs ============
...
// ============ crates/trinity-kernel/src/brain_desktop.rs ============
...
// ============ crates/trinity-kernel/src/brain_quadradical.rs ============
...
// ============ crates/trinity-kernel/src/config.rs ============
...
// ============ crates/trinity-kernel/src/device.rs ============
//! Device Detection
//!
...
// ============ crates/trinity-kernel/src/input_injector.rs ============
...
// ============ crates/trinity-kernel/src/memory.rs ============
...
// ============ crates/trinity-kernel/src/npu_backend.rs ============
...
use crate::runtime::{AutonomousTask, TaskType};
        ...
        match &task.task_type {
            ...
            TaskType::GenerateCode {
                ...
                let code_prompt = format!(
                    "Generate {} code for the following request. Output ONLY the code, no explanations:\n\n{}",
                    language, prompt
                );

                let code = brain.think(&code_prompt).await?;
            ...
            TaskType::EditFile { path, instructions } => {
                ...
                let edited = brain.think(&edit_prompt).await?;
            ...
            TaskType::WorkspaceScan { path } => {
                let _ = event_tx.send(AgentEvent::Thinking {
                    agent_id: agent_id.to_string(),
                    thought: format!("Scanning workspace: {}", path),
                });
                // Placeholder - would scan for TODOs, issues, etc.
                Ok(format!("Workspace scan of {} complete", path))
            }
...
// ============ crates/trinity-kernel/src/resource.rs ============
...
// ============ crates/trinity-kernel/src/rpc_pool.rs ============
...
// ============ crates/trinity-kernel/src/runtime.rs ============
...
impl AutonomousRuntime {
    ...
    pub fn record_result(&self, task: &AutonomousTask, result: Result<String>, duration: Duration) {
...
// ============ crates/trinity-kernel/src/safety.rs ============
...
// ============ crates/trinity-kernel/src/swappable_brain.rs ============
...
// ============ crates/trinity-kernel/src/systemd_control.rs ============
...
// ============ crates/trinity-kernel/src/system_reaper.rs ============
...
// ============ crates/trinity-kernel/src/task_store.rs ============
...
// ============ crates/trinity-kernel/src/todo_parser.rs ============
...
// ============ crates/trinity-kernel/src/voice.rs ============
...
// ============ crates/trinity-kernel/src/wasm_sandbox.rs ============
...
// ============ crates/trinity-protocol/src/artifact.rs ============
...
// ============ crates/trinity-protocol/src/brain.rs ============
...
// ============ crates/trinity-protocol/src/lib.rs ============
...
// ============ crates/trinity-protocol/src/memory.rs ============
...
// ============ crates/trinity-protocol/src/stream.rs ============
...
// ============ crates/trinity-protocol/src/task.rs ============
...
// ============ crates/trinity-protocol/src/types.rs ============
use serde::{Deserialize, Serialize};
...
// ============ crates/trinity-skills/src/code_editor.rs ============
use anyhow::Result;
use std::path::Path;
use tokio::fs;
...
// ============ crates/trinity-skills/src/coder.rs ============
...
// ============ crates/trinity-skills/src/drive.rs ============
...
// ============ crates/trinity-skills/src/lib.rs ============
...
// ============ crates/trinity-skills/src/media/mod.rs ============
...
// ============ crates/trinity-skills/src/tools/executor.rs ============
...
// ============ crates/trinity-skills/src/tools/mod.rs ============
...
// ============ crates/trinity-skills/src/web.rs ============
use anyhow::Result;
use reqwest::Client;
...
// ============ crates/trinity-skills/src/writer.rs ============
